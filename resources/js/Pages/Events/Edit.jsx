...
export default function EditEvent({ event, user, platforms }) {
    const [preview, setPreview] = useState(
        event.image ? `/storage/events/${event.image}` : null
    );
    const initialRows = (event.platform_links || []).map((p) => ({
        event_platform_id: p.event_platform_id ?? "",
        link: p.link ?? "",
    }));

    const [platformRows, setPlatformRows] = useState(
        initialRows.length ? initialRows : [{ event_platform_id: "", link: "" }]
    );
    ...
    const submit = (e) => {
        ...
        post("/events/update", { forceFormData: true });
    };
        ...
        <AuthenticatedLayout user={user}>
            <div className="max-w-3xl mx-auto">
                <div className="flex justify-between items-center mb-4">
                    <h4 className="text-xl font-bold text-white">Edit Event</h4>
                </div>
